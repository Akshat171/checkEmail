...
//       return {
//         email: email,
//         classification: data.content[0].parts[0].text.trim(), // Adjust this line based on the actual response structure
//       };
//     } catch (error) {
...
async function classifyEmails(emails, apiKey) {
  const classificationPromises = emails.map(async (email) => {
    try {
      ...
      // Adjust this line based on the actual response structure from Gemini API
